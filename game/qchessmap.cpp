...
void QChessMap::mousePressEvent(QMouseEvent *event)
{
    ...
    if(event->button() == Qt::LeftButton)
    {
        ...
        if(event->x() <= 80 && event->x() >= 20)
        {
            column = 0;
        }else if(event->x() >= 137 && event->x() <= 190)
        {
            column = 1;
        }else if(event->x() >= 247 && event->x() <= 300)
        {
            column = 2;
        }else if(event->x() >= 355 && event->x() <= 415)
        {
            column = 3;
        }else if(event->x() >= 470 && event->x() <= 530)
        {
            column = 4;
        }else
            column = -1;

        if(event->y() >= 18 && event->y() <= 50)
        {
            row = 0;
        }else if(event->y() >= 75 && event->y() <= 100)
        {
            row = 1;
        }else if(event->y() >= 125 && event->y() <= 155)
        {
            row = 2;
        }else if(event->y() >= 180 && event->y() <= 205)
        {
            row = 3;
        }else if(event->y() >= 235 && event->y() <= 260)
        {
            row = 4;
        }else if(event->y() >= 290 && event->y() <= 315)
        {
            row = 5;
        }else if(event->y() >= 460 && event->y() <= 485)
        {
            row = 6;
        }else if(event->y() >= 515 && event->y() <= 538)
        {
            row = 7;
        }else if(event->y() >= 567 && event->y() <= 595)
        {
            row = 8;
        }else if(event->y() >= 625 && event->y() <= 650)
        {
            row = 9;
        }else if(event->y() >= 677 && event->y() <= 704)
        {
            row = 10;
        }else if(event->y() >= 730 && event->y() <= 755)
        {
            row = 11;
        }else
            row = -1;
        ...
        if(movePoint.size() == 1)
        {
            ...
            if(tempChess1->getType() == CHESS_FALG || tempChess1->getType() == CHESS_MINE)
            {
                unselect(2);
                return ;
            }
...
void QChessMap::sendMoveQi()
{
    ...
    */
    ...
    if((tempChess1->getType() >= tempChess2->getType()) && (tempChess1->getType() != CHESS_BOMB || tempChess2->getType() != CHESS_BOMB)
            && (tempChess1->getType() != CHESS_Engineer))
    {
        qDebug() << "级别不够";
        unselect(1);
        return ;
    ...
    //工兵特判
    if(tempChess1->getType() == CHESS_Engineer)
    {
        if(tempChess2->getType() != CHESS_FALG && tempChess2->getType() != CHESS_MINE && tempChess2->getType() != CHESS_BOMB)
        {
            unselect(1);
            return ;
        }
    ...
    //如果是炸弹的话，要两个都消失
    if(tempChess1->getType() == CHESS_BOMB || tempChess2->getType() == CHESS_BOMB)
    {
        IsBomb = true;
    }
...
void QChessMap::updateChess(QString mes)
{
    int x,y;
    QStringList s = mes.split("&");
    x = s[0].toInt();
    y = s[1].toInt();
    ...
    for(int i = 0;i < m_chess.size();i++)
    {
        if(m_chess[i]->getPosx() == x && m_chess[i]->getPosy() == y)
        {
            //qDebug() <<  GetChessCampIntoString(m_chess[i]->getCamp()) + "_" + GetChessTypeIntoString(m_chess[i]->getType());
            emit updateLabel(GetChessCampIntoString(m_chess[i]->getCamp()) + "_" + GetChessTypeIntoString(m_chess[i]->getType()));
        }
    }
}
